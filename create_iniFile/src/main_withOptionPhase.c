...
{ 
  ...
  else
  {
    ...
    useOnlyPhase = atoi(argv[15]);
    n = atoi(argv[16]);
    equilateral = atoi(argv[17]);
    k1 = atof(argv[18]);
    k2 = atof(argv[19]);
    numValues = atoi(argv[20]);
    theta = atof(argv[21]);
    kbinwidth = atof(argv[22]);
    strcpy(kbinningCase, argv[23]);
    dcosTheta = atof(argv[24]);
    
    strcpy(output_dir, argv[25]);
    strcpy(output_basename, argv[26]);
    write_numpolygons = atoi(argv[27]);
  }
